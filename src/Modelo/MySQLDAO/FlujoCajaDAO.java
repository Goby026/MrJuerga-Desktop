 ...
 * @author Marce
 */
public class FlujoCajaDAO extends Conexion implements DAO<FlujoCaja> {
    ...
    /* METODO PARA OBTENER EL ID DE FLUJO DE CAJA CON FECHA INICIAL E ID DE CAJA PARA MOSTRAR LOS DATOS DE VENTAS POR DIA  */
    public int getIdFlujo(String fecha_inicio, int idCaja) throws Exception {
        try {
            this.conectar();
            PreparedStatement pst = this.conexion.prepareStatement("select idflujocaja\n"
                    + "        from flujocaja\n"
                    + "        where fecha_inicio = ? and idcaja = ?");
...
//                            + "where npbarra.idflujocaja = " + idFlujoCaja + " and operacion_np.tarjeta = \"VISA\"");
    ...
    //obtener un flujo de caja a partir de su id
    public FlujoCaja Obtener(String fecha_inicio, int idCaja) throws Exception {
        FlujoCaja fc = new FlujoCaja();
        try {
            this.conectar();
            PreparedStatement pst = this.conexion.prepareStatement("SELECT * FROM flujocaja WHERE fecha_inicio = ? AND idcaja = ?");
            pst.setString(1, fecha_inicio);
            pst.setInt(2, idCaja);
            ResultSet res = pst.executeQuery();
            while (res.next()) {

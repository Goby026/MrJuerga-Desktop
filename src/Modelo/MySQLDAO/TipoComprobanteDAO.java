...
public class TipoComprobanteDAO extends Conexion implements TipoComprobanteCRUD {
    ...
    public int getIdComprobante(String comprobante) throws Exception {
        int idComprobante = 0;
        try {
            this.conectar();
            PreparedStatement pst = this.conexion.prepareStatement("SELECT idtipocomprobante FROM tipocomprobante WHERE descripcion =  '"+comprobante+"'");            
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                idComprobante = rs.getInt("idtipocomprobante");
            }
            rs.close();
            pst.close();
        } catch (Exception e) {

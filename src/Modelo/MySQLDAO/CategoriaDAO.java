...
public class CategoriaDAO extends Conexion implements CategoriaCRUD {
    ...
    public Categoria obtenerCategoria(int idCategoria) throws SQLException, Exception {
        Categoria cat = new Categoria();
        try {
            this.conectar();
            PreparedStatement pst = this.conexion.prepareStatement("SELECT descripcion,Codcategoria  FROM categoria WHERE idcategoria = " + idCategoria);
            ResultSet res = pst.executeQuery();
            if (res.next()) {
                cat.setDescripcion(res.getString("descripcion"));
                cat.setCodcategoria(res.getString("Codcategoria"));

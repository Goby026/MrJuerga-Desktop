...
public class UsuarioDAO extends Conexion implements UsuarioCRUD{
    ...
    //obtener usuario a partir del nombre de usuario
    public Usuario Obtener(String usuario) throws Exception {
        Usuario u = null;
        try {
            this.conectar();
            PreparedStatement pst = this.conexion.prepareStatement("select * from usuario where usuario = ?");
    ...
    public int getIdUsuario(String usuario) throws Exception {
        ...
        try {
            this.conectar();
            PreparedStatement pst = this.conexion.prepareStatement("SELECT idusuario FROM usuario WHERE usuario =  '"+usuario+"'");            
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
